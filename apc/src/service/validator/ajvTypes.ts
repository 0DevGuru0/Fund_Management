/* eslint-disable */
/*
  this file is a modified version of `avj/dist/types/json-schema.d.ts`
  why?
  1. to make `required` field optional
  2. `_partial` parameter should have effect on nested fields
*/
...
declare type JSONType<T extends string, _partial extends boolean> = _partial extends true
  ? T | undefined
  : T;
...
export declare type JSONSchemaType<
  ...
  _openAPIQueryParam extends boolean = false
> = (T extends number
  ? {
  ...
  ? {
      type: JSONType<'array', _partial>;
      items: JSONSchemaType<T[0], _partial>;
  ...
  ? {
      ...
      required?: _partial extends true ? (keyof T)[] : RequiredMembers<T>[] | readonly RequiredMembers<T>[];
      additionalProperties?: boolean | JSONSchemaType<T[string]>;
      unevaluatedProperties?: boolean | JSONSchemaType<T[string]>;
      // properties?: _partial extends true
      //   ? Partial<PropertiesSchema<T>>
      //   : PropertiesSchema<T>;
      properties?: _partial extends true
        ? Partial<PropertiesSchema<T, _partial, _openAPIQueryParam>>
        : PropertiesSchema<T, _partial, _openAPIQueryParam>;
      patternProperties?: {
...
//  [K in keyof T]-?:
//   | (JSONSchemaType<T[K]> & Nullable<T[K]>)
declare type PropertiesSchema<T, _partial extends boolean, _openAPIQueryParam extends boolean> = {
  [K in keyof T]-?:
    JSONSchemaType<T[K], _partial> & Nullable<T[K]> & (_openAPIQueryParam extends true ? { $ref?: string; openApiIn: 'query' | 'path';} : { $ref?: string; });
};
